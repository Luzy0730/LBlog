  ...
  <div class="ui segment m-box m-padded-tb-large m-margin-bottom-big" v-if="blogItem">
    <div class="ui middle aligned mobile reversed stackable">
      <div class="ui grid m-margin-lr">
        ...
        <div class="row m-padded-tb-small">
          <div class="ui horizontal link list m-center">
            <div class="item m-datetime">
              <i class="small calendar icon"></i><span>{{ timeFormat(blogItem.create_time) }}</span>
            </div>
        ...
        <!--分类-->
        <router-link :to="`/category/${blogItem.category.name}`" :style="colorStyle(blogItem.category.color)" class="ui large ribbon label m-hover-dark">
          <icon :name="blogItem.category.icon" /><span class="m-text-500 m-margin-lr">{{ blogItem.category.name
          }}</span>
        </router-link>
        <!--文章Markdown 描述 | 内容-->
        <div class="typo m-padded-tb-small line-numbers" ref="docRef" v-html="isDetail ? blogItem.content : blogItem.description">
        </div>
        <!--阅读全文按钮-->
        <div class="row">
        ...
        <div class="row m-padded-tb-no">
          <div class="column m-padding-left-no">
            <router-link :to="`/tag/${tag.name}`" class="ui tag label m-text-500 m-margin-small m-hover-dark" :style="colorStyle(tag.color)" v-for="tag in blogItem.tags" :key="tag.id">{{ tag.name }}</router-link>
          </div>
        </div>
      </div>
    </div>
